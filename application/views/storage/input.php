        ...
        <div class="row gx-5">
            <div class="col-lg-6">
                <div class="bg-white text-center rounded p-5">
                    <h1 class="mb-4">Input Detail Produk</h1>
                    <form method="post" action="<?= base_url('storage/proses/' . $this->uri->segment(3)); ?>">
                        <div class="row g-3">
                            <div class="col-12">
                                <label style="float: left; " for=""><b>Nama Produk</b> </label>
                                <select class="form-select bg-light border-0" style="height: 55px;" name="id_produk">
                                    <option disabled selected>Pilih Nama Produk</option>
                                    <?php foreach ($produk as $p) {
                                        $nama_produk = $p->nama_produk;
                                        $id_produk = $p->id_produk;
                                        $sub_kategori = $p->nama_sub_kategori;
                                        $berat = $p->berat_produk / 1000;
                                        ?>
                                        <option value="<?= $id_produk ?>"><?= $sub_kategori ?> - <?= $nama_produk ?></option>
                                    <?php } ?>
                                </select>
            ...
            <div class="col-lg-6 mb-5 mb-lg-0">
                ...
                <!-- <a href="<?= base_url('storage/pilih_kategori') ?>"><button class="btn btn-primary py-3">Buat Nama dan
